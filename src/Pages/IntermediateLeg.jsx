...
const IntermediateLeg = () => {
 ...
 const initialExercises = [
  ...
  { name: "Hamstring Curl", done: false },
  { name: "Calf Raises", done: false }
];

  const [exercises, setExercises] = useState(() => {
    const saved = localStorage.getItem("legWorkout");
    return saved ? JSON.parse(saved) : initialExercises;
  });

  // ✅ SAVE
  useEffect(() => {
    localStorage.setItem("legWorkout", JSON.stringify(exercises));
  }, [exercises]);
  ...
  const handleTimer = () => {
    if (!running) {
      ...
      localStorage.removeItem("legWorkout");
      ...
      {showPopup && (
        <div className="popup-overlay">
          ...
          <div className="popup-box">
            ...
            <p>Don’t skip leg day next time 😤🔥</p>
